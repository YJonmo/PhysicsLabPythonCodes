...
def DAQ_Speed_Test(No_DAQ_Tests):
    ...
    print ('DAQ Analogue reading requires %f s per sample' %Mean_Time)
...
def Power_Speed_Test(No_Power_Tests):
    ...
    print ('Power meter reading requires %f s per sample' %Mean_Time)
...
def Spec_Speed_Test(No_Spec_Tests):
    ...
    while True:
        Start_Time = time.time()
        Spec_Read_Process(No_Spec_Tests, 0)
        '''
...
def Spec_Read_Process(No_Spec_Sample, Count_Yes_No):
    I = 0   
...
def Multi_Integration_Paradigm(Integration_list_MilSec, Integration_Buffer_Time, Shutter_Delay, No_Power_Sample):
    ...
    Pros_Spec = Process(target=Spec_Read_Process, args=(len(Integration_list_MilSec), 0))
    Pros_Spec.start()    
    ...
    Timer_Is_Over.value = 0           
    P_Timer = Process(target=Timer_Multi_Process, args=(Integration_Buffer_Time,))
    P_Timer.start()
...
def Continious_Paradigm(Integration_Continious, No_Spec_Sample, No_DAC_Sample, No_Power_Sample):
    ...
    while (int(Spec_Index[0]) < No_Spec_Sample - 10 ):
        ...
        if  Spec_Is_Read.value == 1:
            Spec_Is_Read.value = 0
            '''
            Local_Spec_Index = Local_Spec_Index + 1
            if (Local_Spec_Index != int(Spec_Index[0])):
                Full_Spec_Records[:, np.int(Spec_Index[0])-1] = Last_Spec_Record[:]
                Local_Spec_Index = np.int(Spec_Index[0])
            '''
