...
def SaveData(TimeIndex, Power):                          # This function save the recorded date in the HDF5 format. You don't need to call it when using for testing.
    ...
    Spec_subgroup1 = file.create_group("ThorlabsPM100")
    Spec_intensities = file.create_dataset('ThorlabsPM100/Power', data = Power)
    Spec_wavelength = file.create_dataset('ThorlabsPM100/TimeIndex', data = TimeIndex)
    #dset.attrs["attr"] = b"Hello"
    #Spec_subgroup1.attrs['ThorlabsPM100 Details'] = np.string_(ThorlabsPM100.getDetails())
    file.close()
...
for I in range(No_Power_Sample):
    try:
        ...
        Read_Power[I] = Power_meter.readPower()
